...
)  # NOTE: Vale la pena ponerlo a 0 para todos los registros?
async def get_reports(
    response: Response,
    page: int = Query(0, ge=0, description="Pagina actual"),
    ...
    await _set_pagination_header(
        response, page_size, total_items
    )  # Añadir a la cabecera el total paginas e items

    return PaginatedReportsResponse(page=page, page_size=page_size, reports=reports)
...
async def update_report(
    id: int, report_data: ReportUpdate, db: AsyncSession = Depends(get_db)
):
...
)  # TODO: Al borrar un reporte, borrar sus imagenes y comentarios asociados
...
@router.post(
    "/{id}/images", status_code=status.HTTP_201_CREATED
)  # TODO: Añadir limites de cantidad y validaciones de tipo de archivo
async def upload_report_image(
    file: UploadFile = File(...),
    report: Report = Depends(report_dependencies.valid_report_id),
    db: AsyncSession = Depends(get_db),
    current_user=Depends(auth_dependencies.GetCurrentUser()),
):
    saved_files = await report_services.save_images(
        [file], report.id, current_user[0].id, db
    )
    return {
        "message": f"Imagen subida para el reporte {report.title} | {report.id}",
...
async def delete_all_report_images(
    report: Report = Depends(report_dependencies.valid_report_id),
    db: AsyncSession = Depends(get_db),
):
...
async def create_report_comment(
    ...
    comment = await report_services.create_report_comment(
        report.id, comment_data, current_user[0].id, db
    )
    return comment
...
async def delete_report_comment(
    ...
    await report_services.delete_report_comment(
        report.id, comment_id, current_user[0].id, db
    )
    return Response(status_code=status.HTTP_204_NO_CONTENT)
