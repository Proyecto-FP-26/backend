...
async def authenticate_user(
    auth_data: UserLogin, db: AsyncSession, response: Response, request: Request
):
...
async def refresh_tokens(
    user_jti: Tuple[User, str], db: AsyncSession, response: Response, request: Request
):
    user, jti = user_jti
    ...
    if session:
        if (
            session.isRevoked
        ):  # Si el jti enviado estaba revokado, no se crean nuevos tokens
            raise SIN_CREDENCIALES
...
async def _create_auth(
    response: Response, request: Request, db: AsyncSession, user: User
):
