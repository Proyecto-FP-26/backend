...
from app.models.report import Comment as ReportComment
...
def report_with_relations():  # Se cargan las relaciones desde el modelo
    return [
        selectinload(
            Report.user
        ),  # selectinload carga la relación en la misma consulta(Pero ejecuta más de una query) : Select * from report; Select * from user where user.id in (1,2,3)
...
async def get_all_reports(page: int, page_size: int, db: AsyncSession):
    ...
    if page == 0 or page_size == 0:
        main_query = (
            select(Report).options(*report_with_relations()).order_by(Report.id)
        )
    else:
...
async def get_report_by_id(report_id: int, db: AsyncSession):
    main_query = (
        select(Report).where(Report.id == report_id).options(*report_with_relations())
    )
    result = await db.execute(main_query)
...
async def create_new_report(report_data: ReportCreate, db: AsyncSession):
    ...
    )  # Convierte ReportCreate a diccionario y lo pasa al constructor de Report

    db.add(new_report)  # pendiente enviar db
    await db.commit()  # confirma y guarda en db
    await db.refresh(
        new_report
    )  # actualiza con los datos de la db generados automaticamente
...
async def update_report_by_id(
    report_id: int, report_data: ReportUpdate, db: AsyncSession
):
    ...
    update_data = report_data.model_dump(
        exclude_unset=True
    )  # model_dump: convierte el modelo en un diccionario | exclude_unset : Actualiza solo los campos que vienen en el body, ignora los None (sin esto, actualizaría todos los campos no enviados a None (valor por defecto de ReportUpdate))
    ...
    result = await db.execute(
        select(Report).options(*report_with_relations()).where(Report.id == report_id)
    )
...
async def save_images(
    files: list[UploadFile], report_id: int, user_id: int, db: AsyncSession
):
    ...
    for file in files:
        ...
        if file.content_type not in allowed_mimes:
            saved_files.append(
                f"Archivo {file.filename} con tipo {file.content_type} no permitido"
            )  # HACK: Temporal, cambiar por mensaje de error o algo similar
...
async def get_report_images(report_id: int, db: AsyncSession):
    result = await db.execute(
        select(ReportImage).where(ReportImage.reportId == report_id)
    )
...
async def delete_report_image(report_id: int, image_id: int, db: AsyncSession):
    result = await db.execute(
        select(ReportImage).where(
            ReportImage.id == image_id, ReportImage.reportId == report_id
        )
    )
...
async def create_report_comment(
    report_id: int, comment_data: str, user_id: int, db: AsyncSession
):
    new_comment = ReportComment(
        reportId=report_id, content=comment_data, userId=user_id
    )
    db.add(new_comment)
    await db.commit()
